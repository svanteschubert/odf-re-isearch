...
public class JarManifestIT {
  ...
  //
  //
  //            File jarFile = getFileFromSubstring("target" + File.separator,
  // "-jar-with-dependencies.jar");
  //            System.err.println("JarFile: " + jarFile.getAbsolutePath());
  //			String command = "java -jar target" + File.separator +
  // "odfdom-java-0.9.0-incubating-SNAPSHOT-jar-with-dependencies.jar";
  ...
  //			LOG.log(Level.INFO, "\"{0}\"", firstOutputLine);
  //			LOG.log(Level.INFO, "\"{0}\"", secondOutputLine);
  //			Assert.assertEquals(firstOutputLine, JarManifest.getOdfdomTitle() + " (build " +
  // JarManifest.getOdfdomBuildDate() + ')');
  //			Assert.assertEquals(secondOutputLine, "from " + JarManifest.getOdfdomWebsite() + " supporting
  // ODF " + JarManifest.getOdfdomSupportedOdfVersion());
  //
  //			LOG.log(Level.INFO, "\nJarManifest.getName(): {0}", JarManifest.getOdfdomName());
  //			Assert.assertNotNull(JarManifest.getOdfdomName());
  //
